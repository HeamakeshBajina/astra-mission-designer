"""3D finite-thrust propagation with Earth's J2 perturbation for ASTRA."""
...
class State3DThrustJ2:
    """Spacecraft state with 3D position, velocity, and mass."""
...
def _derivatives(
    ...
    """Return derivatives from gravity, J2, and finite thrust."""
    ...
    ax_j2, ay_j2, az_j2 = j2_acceleration(
        x=x,
        y=y,
        z=z,
    )

    # Finite-thrust acceleration.
    thrust_acceleration = (
        thrust_model.thrust / state.mass
    )

    ax_thrust = thrust_acceleration * ux
    ay_thrust = thrust_acceleration * uy
    az_thrust = thrust_acceleration * uz

    return State3DThrustJ2(
        x=state.vx,
        y=state.vy,
        z=state.vz,
        vx=ax_gravity + ax_j2 + ax_thrust,
        vy=ay_gravity + ay_j2 + ay_thrust,
        vz=az_gravity + az_j2 + az_thrust,
...
def rk4_step_thrust_j2(
    ...
    """Advance a J2 + finite-thrust state by one RK4 timestep."""
    ...
    # Validate thrust direction before integration.
